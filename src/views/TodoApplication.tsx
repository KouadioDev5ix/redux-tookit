...
import {
  ...
  supprimerTâche,
  modifierTâche,
} from "../features/todosSlices/todosSlices";
...
export default function TodoApplication() {
  ...
  const fruits = new Map([["Pomme", 400]]);
  ...
  for (const [cle, valeur] of users) {
    console.log(cle, Object.entries(valeur).flat());
  }

  useEffect(() => {
    console.log("Les utilisateurs", users);
    console.log("Utilisateur par id ", userParId);
  }, []);
      ...
      <div className="flex flex-col gap-4 w-10/12 max-w-md mx-auto">
        ...
        <div className="flex gap-2">
          <input
            ...
            type="text"
            className="w-full h-8 border border-orange-600 rounded-md focus:outline-none px-2 text-sm "
            placeholder="Ajouter une tâche..."
          />
          <button
            className="w-24 h-8 bg-blue-900 text-white text-xs rounded-md"
            onClick={() => {
              if (modeEdition) {
